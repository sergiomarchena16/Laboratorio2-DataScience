...
modeloLinealSimple<-lm(SalePrice~OverallQual, data = trainSet)
...
# niveles. 
modeloLinealMulti<-lm(as.numeric(SalePrice)~., data = nums)
...
prede<-as.factor(testSet$SalePred)
str(prede)
cfm<-confusionMatrix(as.factor(real),testSet$SalePred)
cfm
